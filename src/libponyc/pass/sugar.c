...
static ast_result_t sugar_with(pass_opt_t* opt, ast_t** astp)
{
  ...
  // You can use `with` without throwables so we don't have to have a
  // throwable in the generated try/then
  // However, in order to use the result of a `with` in an assignment expression
  // we have to use an `else` so we don't get an autogenerated `else None`
  // added.
  // Really what we want it to be able to know at this time if the with body
  // contains a throwable and do the right thing but that isn't available at
  // this point.
  // We should be being more smart here but this is a fix for #3759 until
  // something deeper can be done.
